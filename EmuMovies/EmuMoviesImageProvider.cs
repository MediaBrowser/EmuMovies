...
namespace EmuMovies
{
    public class EmuMoviesImageProvider : IRemoteImageProvider, IHasOrder, IHasMetadataFeatures
    {
        ...
        {
            var sessionId = await Plugin.Instance.GetEmuMoviesToken(cancellationToken).ConfigureAwait(false);

            var list = new List<RemoteImageInfo>();

            if (string.IsNullOrEmpty(sessionId)) return list;

            var gameSystem = game.FindParent<GameSystem>();

            var consoleType = gameSystem?.Container;
